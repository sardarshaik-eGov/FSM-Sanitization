...
public class WorkflowService {
	...
	public Integer getNearingSlaProcessCount(String tenantId, RequestInfo requestInfo,
	...
	public List<HashMap<String, Object>> getProcessStatusCount(RequestInfo requestInfo,
		...
		for (String businessSrv : listOfBusinessServices) {
			...
			if ((!ObjectUtils.isEmpty(criteria.getModuleName())
					&& !criteria.getModuleName().equalsIgnoreCase(FSMConstants.BPA))
					|| (!ObjectUtils.isEmpty(criteria.getModuleName())
							&& criteria.getModuleName().equalsIgnoreCase(FSMConstants.BPA)
							&& !roles.contains(FSMConstants.CITIZEN)))
				url = this.buildWorkflowUrl(criteria, url, Boolean.FALSE);
			...
			if (criteria != null && !ObjectUtils.isEmpty(criteria.getModuleName())
				...
				if (finalResponse == null) {
					...
					finalResponse.addAll((List<HashMap<String, Object>>) serviceRequestRepository.fetchListResult(url,
							requestInfoWrapper));
				}
			}
		}
	 ...
	 * @param businessServiceName businessService code
	...
	private StringBuilder buildWorkflowUrl(ProcessInstanceSearchCriteria criteria, StringBuilder url,
			boolean noStatus) {
		...
		if (criteria.getIsProcessCountCall() || ObjectUtils.isEmpty(criteria.getModuleName())
				&& !StringUtils.isEmpty(criteria.getBusinessService())) {
			url.append("&businessService=")
					.append(StringUtils.arrayToDelimitedString(criteria.getBusinessService().toArray(), ","));
		}
					...
					businessServicesByTenantId.forEach(service -> {
						...
						states.forEach(state -> {
							...
							if (!CollectionUtils.isEmpty(stateRoles)
									&& !Collections.disjoint(stateRoles, entry.getValue())) {
	...
	/**
	 * Creates a map of status to roles who can take actions on it for all
	 * businessService
	 * 
	 * @param businessServices The list of businessServices
	 * @return Map of status to roles which can take action on it for all
	 *         businessService
	 */
